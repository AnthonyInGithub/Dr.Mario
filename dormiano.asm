...
horizontal_check_loop:
    ...
    mul $t3, $s0, 25                  # t3 = row * num_columns
...
match_horizontal_blocks:
    ...
    mul $t6, $s0, 25                  # t6 = row * num_columns
...
apply_gravity:
    ...
    sw $s3, 16($sp)  # Current block
    sw $s4, 20($sp)  # Neighbor block
    sw $s5, 24($sp)  # Direction flag
    sw $s6, 28($sp)  # Temporary calculation

    la $t0, current_map      # Load current_map base address
    la $t1, direction_map    # Load direction_map base address
    li $s0, 0                # Start at column 0
    li $t2, 22               # Number of columns
    li $t3, 25               # Number of rows

gravity_column_loop:
    bge $s0, $t2, gravity_done  # If column exceeds max, finish
    li $s1, 24                  # Start from the bottom row

gravity_row_loop:
    bltz $s1, next_column       # If row index is below 0, move to the next column

    # Calculate the current block's address in current_map
    mul $s6, $s1, 22            # s6 = row * num_columns
    add $s6, $s6, $s0           # s6 = (row * num_columns) + column
    sll $s6, $s6, 2             # Convert to byte offset
    add $s6, $t0, $s6           # Address in current_map
    lw $s3, 0($s6)              # Load current block color

    # Calculate the corresponding address in direction_map
    mul $s4, $s1, 22            # s4 = row * num_columns
    add $s4, $s4, $s0           # s4 = (row * num_columns) + column
    sll $s4, $s4, 2             # Convert to byte offset
    add $s4, $t1, $s4           # Address in direction_map
    lw $s5, 0($s4)              # Load direction value

    # Skip if the block is empty
    beq $s3, $zero,  move_to_next_row

    # Handle blocks based on direction
    beq $s5, 5, move_to_next_row
    beq $s5, $zero, handle_independent_block   # Independent block
    beq $s5, 1, handle_right_connected_block   # left and right capsuele
    beq $s5, 4, handle_bottom_connected_block
    
    j move_to_next_row

handle_independent_block:
    # Gravity logic for independent blocks
    add $s2, $s1, $zero
    addi $s2, $s2, 1                   # Target row below

    bgt $s2, 24,  move_to_next_row      # If below is out of bounds, clear the block
    mul $s6, $s2, 22
    add $s6, $s6, $s0
    sll $s6, $s6, 2
    add $s6, $t0, $s6
    lw $s4, 0($s6)                      # Load block below

    beq $s4, $zero, down_by_gravirty # Stop if there's a block below
    jr $ra
    
    handle_right_connected_clocl:

    # Check if both parts of the pill can move
    sub $s6, $s0, 1                     # Left column
    bltz $s6, move_to_next_row          # Skip if out of bounds
    mul $s7, $s1, 22
    add $s7, $s7, $s6
    sll $s7, $s7, 2
    add $s7, $t0, $s7
    lw $s4, 0($s7)                      # Load left block

    # Ensure space below both parts
    addi $s2, $s1, -1
    mul $s7, $s2, 22
    add $s7, $s7, $s0
    sll $s7, $s7, 2
    add $s7, $t0, $s7
    lw $s4, 0($s7)                      # Load below block

    bnez $s4, move_to_next_row          # Stop if space is occupied

    # Move both parts down
    sw $s3, 0($s7)                      # Move current block down
    sw $zero, 0($s6)                    # Clear original position
    jr $ra
...
handle_pill_below:
    # Handle pill connected below
    jr $ra

clear_current_block:
    # Clear block in current_map and display
    sw $zero, 0($s6)                    # Clear in current_map
    lw $s6, ADDR_DSPL
    mul $s4, $s1, 128
    mul $s3, $s0, 4
    add $s6, $s6, $s4
    add $s6, $s6, $s3
    sw $zero, 0($s6)                    # Clear in display
    jr $ra
